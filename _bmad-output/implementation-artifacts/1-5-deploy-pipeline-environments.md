...
- [ ] [Review][Patch] `wranglerConfig.test.tsx`'s vars-block regex (`/"vars"\s*:\s*\{([^}]*)\}/g`) truncates early if a var's string value ever contains a literal `}`, silently missing any keys declared after it. Unreached today (current values are plain strings), but a latent gap in a security-relevant test. **Acknowledged, not fixed** — the same disproportionate-effort call as the vars-allowlist finding above (a real fix needs the same JSONC-parsing approach); left as an open, low-priority item rather than silently dropped [`src/shared/lib/wranglerConfig.test.tsx:34`]
