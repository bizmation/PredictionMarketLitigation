...
4. **When** authenticated as the operator, **then** the admin shell renders the `.adminbar` session strip with the operator display name per the UX admin bar, and the resolved identity is available to server code for later Evidence attribution
...
env.d.ts · wrangler.jsonc · wrangler.test.jsonc  # MODIFIED — Env types, run_worker_first
```
...
- **DEVIATION from Task 4, deliberate and safer.** The story said to declare `ACCESS_DEV_BYPASS` in `wrangler.jsonc` top-level `vars`. I put it in `.dev.vars` instead. Reason: no Wrangler `env` blocks exist yet, so a plain `wrangler deploy` — exactly what `npm run deploy` runs — would have carried a top-level var straight to production with the bypass on. `.dev.vars` is gitignored (`.dev.vars*`) and read only by `wrangler dev`: never bundled, never deployed. This makes shipping the bypass enabled *structurally impossible* rather than a convention Story 1.5 must remember. AC7 is satisfied more strongly, not less.
...
- **107 tests pass across 5 files, exit 0, with zero cloud credentials** (baseline was 67 across 4). 26 new in `access.test.ts`, 11 new in `server.test.ts`, 5 new in `shells.test.tsx`. `npx tsc` and `npx oxlint src/` both clean.
