...
4. ☑ **FULLY SATISFIED 2026-08-10** (display-name half closed in Story 1.5 Part B). **When** authenticated as the operator, **then** the resolved identity is available to server code for later Evidence attribution, and the admin shell renders the `.adminbar` session strip per the UX admin bar. *(Scope was reduced by review decision 2026-08-10 — rendering the display name moved to Story 1.5, because until Access issued a real session a session endpoint would only ever echo the local dev-bypass stub.* **That condition was met the same day:** *the Access application went live, and `GET /api/admin/session` plus `useAdminSession` now render the real name. Only `displayName` crosses the wire — never the email, which AC3's Story 3.13 dependency makes load-bearing, since that name gets published in mode-change audits on `ops.`. The strip still reads "Not signed in" whenever the server has not verified an identity, including first paint and expired sessions.)*
