...
- Story 3.19: Epic 3 Hardening — Timeouts, Gateway Seed & Deferred Closure (added 2026-09-17)
- Story 3.20: OpenRouter Provider via AI Gateway (added 2026-09-17)
- Story 3.21: First Live Connector (added 2026-09-17)
...
- Evidence is an append-oriented projection (`run.started`, `source.fetched`, `draft.created`, `gate.awaiting_approval`, `run.stopped`, plus `steering.turn` / `steering.applied` / `steering.denied`, `draft.revised`, `config.steered`, `guidance.recorded` / `guidance.revoked`). Spend as integer cents.
...
- Decided since planning: standing-guidance cap 12 items / 600 chars (3.18); initial role→model pin is Workers AI for all five roles with a 500-cent default budget via migration 0017, OpenRouter deferred to 3.20; timeouts 15 s client GET / 30 s admin POST / 60 s connector + provider (sprint-change-proposal-2026-09-17 §4.1).
- Unresolved in planning (do not invent): numeric YOLO threshold beyond the current code value; whether to cap revision-chain depth; exact UTC cron for noon ET.

## UX & Interaction Patterns

- `ops.` is receipts, not a second litigation dashboard. Pending Drafts always carry NotLiveDraftBanner / ticket-edge treatment. Run log uses status chips (published, awaiting, empty, failed, stopped) and origin flags (scheduled, catch-up, manual); next-run is public; rows link to Evidence.
