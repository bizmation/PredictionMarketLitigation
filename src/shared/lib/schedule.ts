...
/**
 * Scheduling-time computation for the daily Run (Story 3.3, moved 3.7).
 *
 * Lives in `shared/` so the public ops. surface can display the same next-run
 * instant the cron uses, without importing pipeline. Pipeline callers keep
 * importing from `pipeline/workflow/schedule.ts`, which re-exports this.
 *
 ...
 * the configured run hour (noon ET).
 */
...
export function nextRunAtUtc(now: Date = new Date()): string {
  ...
  // Intl offset is second-precision; drop leftover ms so noon is exact.
  const instant = new Date(Math.floor(now.getTime() / 1000) * 1000);
  const offset = timezoneOffsetMs(instant);
  const wall = instant.getTime() + offset;
