...
**Live check — 2026-09-18, deploy after PR #36 (story 3.21) via `npm run deploy:build` + HTTP. Do not treat this as a production deploy.**

| Check | Result |
|---|---|
| Worker `pml-build` | version `e54cfc19-313e-4b6e-bd5a-cf80119d8021` (main `b1273e6`, story 3.21); prior `540be6b4` (3.19), `ede146a4` (3.17 + 3.18), `32a0fb9c` (through 3.13); Custom Domain `build.predictionmarketlitigation.com` only; workflow `daily-run`; crons 16:00Z / 17:00Z |
| D1 `pml-build` | id `9e83494a-016e-4bff-9e07-7fca3a159a83`; migrations `0001`–`0018` applied (`0018` `docket_events.kind`/`favors` landed this deploy, verified via `pragma_table_info`; `0017` seed row verified earlier: version 1, 500¢, all five roles → Workers AI; `0005` poll votes and Epic 3 tables are on staging, not production) |
...
| Apex SPA bundle | contains "donations open soon" (3.19 placeholder state; `DONATE_URL` still `#coffee`) |
| CourtListener connector | live as of this deploy: four case-owned dockets resolve from `sources` (Assad/9th 72237443 baseline 2025-11-28; Illinois 73133459 2026-04-02; New York 73242633 2026-04-24; Furcolo 73375343 2026-05-21); `COURTLISTENER_API_TOKEN` present on `pml-build`. **First live Run pending Patrick's `Run now`** — record the Run id and what `source.fetched` / `draft.evaluated` showed here. Workers AI Neurons consumed before that Run: 0. |
...
2. **Catch `pml-build` up** (`npm run deploy:build`) so the Worker and D1 `0001`–latest match `main`. Smoke `build.` end-to-end. (Done through `0018` / version `e54cfc19` on 2026-09-18 from main `b1273e6`; re-run if `main` has moved.)
