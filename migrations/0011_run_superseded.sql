-- Story 3.12 — admit `run.superseded` on evidence_events.event.
--
-- D1/SQLite cannot ALTER a CHECK constraint in place (0008/0009 precedent:
-- later stories extend a closed enum by rebuilding so the CHECK stays
-- truthful). Rebuild `evidence_events` with the 0009 definition plus the new
-- event, copy rows, then restore the unique (run_id, seq) index.
--
...
CREATE TABLE evidence_events_new (
  ...
  event        TEXT NOT NULL CHECK (event IN ('run.started','run.completed','run.failed','run.stopped','run.empty','run.superseded','source.fetched','source.skipped','draft.created','draft.evaluated','guardrails.passed','guardrails.failed','gate.awaiting_approval','gate.decided')),
