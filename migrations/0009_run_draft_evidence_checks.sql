-- Story 3.1 post-review CHECK tightening.
--
-- D1 cannot ALTER a CHECK in place (0008 precedent). 0006 (and 0008's
-- evidence_events rebuild) may already be applied on local/staging D1, so
-- this migration rebuilds `runs`, `drafts`, and `evidence_events` with the
-- CHECKs 0006 now declares, and copies `llm_calls` because it FKs to `runs`.
-- Greenfield applies (tests) run 0006 → 0008 → this; already-applied DBs
-- pick up the CHECKs here.
--
...
INSERT INTO runs_new (
  id, origin, mode, status, started_at, completed_at,
